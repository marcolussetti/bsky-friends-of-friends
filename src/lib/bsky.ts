...
export async function computeSuggestions(
  ...
  try {
    ...
    throw new Error(
      `Could not find user "${username}". Please check the username is correct.`
    )
  }
  ...
  emitProgress(
    ...
    `Processing ${followedProfiles.length} follows...`
  )
  ...
  for (const [i, user] of followedProfiles.entries()) {
    emitProgress(
      ...
      i + 1,
      followedProfiles.length,
      `Fetching follows for ${user.handle}...`
    )
  ...
  for (const [i, followedUser] of followedUsers.entries()) {
    emitProgress(
      ...
      `Analyzing ${mode === "friends" ? "friends" : "follows"} of ${followedUser.handle}...`
    )
    ...
    if (mode === "friends") {
      const followedUserFollowers = await getFollowers(
        client,
        followedUser.handle
      )
      tracker.completedCalls++
      ...
      candidatePool = followedUser.following.filter((f) =>
        followerDids.has(f.did)
      )
    }
